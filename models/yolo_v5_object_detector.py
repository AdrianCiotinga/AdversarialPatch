...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def non_max_suppression(prediction, logits, conf_thres=0.6, iou_thres=0.45, classes=None, agnostic=False,
        ...
        for xi, (x, log_) in enumerate(zip(prediction, logits)):  # image index, image inference
            ...
            if multi_label:
                ...
                z_1, z_2 = z_.max(1, keepdim=True)
                # log_ = x[:, 5:]
                x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
                log_ = log_[conf.view(-1) > conf_thres]
                z_1 = z_1[z_1.view(-1) > conf_thres]
            # Filter by class
            ...
            z_1 = z_1[i]
            objects_output.append(z_1)
            assert log_[i].shape[0] == x[i].shape[0]
            if (time.time() - t) > time_limit:
                print(f'WARNING: NMS time limit {time_limit}s exceeded')
                break  # time limit exceeded

        return output, logits_output, objects_output
    ...
    def forward(self, img):
        ...
        prediction, logits, z = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
        ...
        return [self.boxes, self.classes, self.class_names, self.confidences], logits, z
